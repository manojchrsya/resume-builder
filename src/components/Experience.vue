      ...
      <div class="entry" v-for="(data, index) in experiences" :key="index">
        ...
        <div class="entry-header" style="margin-bottom:6px;">
          <div class="entry-org" style="font-style:normal; font-weight:700; font-size:13.5px; color:var(--text);">
            <a :href="data.url ? data.url : '#'" class="entry-org" :data.company target="_blank">{{ data.company }}</a> &nbsp;·&nbsp; <span style="font-weight:400; font-size:12px; color:var(--light);">{{ data.location }}</span>
          </div>
          <div class="entry-date" :data.duration>{{ data.duration }}</div>
        </div>
        ...
        <div 
          ...
          :style="designation.current ? 'padding-left:14px; border-left:2px solid var(--accent); margin-bottom:10px;' : 'padding-left:14px; border-left:2px solid #d1d5db; margin-bottom:10px;'">
          <div class="entry-header">
            <div style="display:flex; align-items:center; gap:8px;">
              <div class="entry-role">{{ designation.title }}</div>
              <span v-if="designation.promoted" style="font-size:10px; font-weight:700; background:#e8f0fe; color:#1a3a5c; padding:1px 7px; border-radius:3px; letter-spacing:.5px; text-transform:uppercase;">Promoted</span>
            </div>
            <div class="entry-date">{{ designation.duration }}</div>
          </div>
          <ul>
            <li v-for="(responsibility, responsibilityIndex) in designation.responsibilities" :key="responsibilityIndex" >{{ responsibility }}</li>
          </ul>
        </div>
      </div>
    </div>
